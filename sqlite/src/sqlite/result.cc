...
{
  if (!stmt)
    throw error("attempt to step null sqlite3_stmt");
  ...
  case SQLITE_BUSY:
    throw transaction_failed(status);
  default:
    throw error(stmt);
  }
}
...
result::result(const std::shared_ptr<sqlite3_stmt>& statement) : stmt(statement)
{
  if (!statement) {
...
{
  if (&other == this) {
    throw error("attempt to move into self");
  }
...
std::size_t result::row_modification_count() const
{
  ...
  if (!db) {
    throw error("unable to aquire associated database");
  }
...
result::const_iterator::const_iterator(const std::shared_ptr<sqlite3_stmt>& statement, bool& at_end)
...
bool result::const_iterator::operator==(const result::const_iterator& other) const
{
...
{
  if (end_reached) {
    throw error("attempt to increment past last result");
  }
  end_reached = step_result(stmt);
  current_row = stmt;
  return *this;
